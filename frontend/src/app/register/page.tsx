...
function PasswordStrength({ password }: { password: string }) {
  ...
  const color =
    passed <= 2 ? "#f87171" : passed <= 3 ? "#fb923c" : passed === 4 ? "#facc15" : "#14B8A6";
        ...
        {PASSWORD_RULES.map((rule) => {
          ...
          return (
            <div key={rule.label} className="flex items-center gap-1.5">
              <span className="text-xs" style={{ color: ok ? "#14B8A6" : "#475569" }}>
                {ok ? "✓" : "○"}
              </span>
        ...
        <div className="w-full max-w-sm">
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500">
              Already have one?{" "}
              <Link href="/login" className="transition-colors hover:text-white" style={{ color: "#14B8A6" }}>
                Sign in
              </Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onFocus={(e) => {
                  if (!confirm || password === confirm) e.target.style.borderColor = "#14B8A6";
                }}
                onBlur={(e) => {
