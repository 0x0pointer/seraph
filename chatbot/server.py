...
API_URL = os.getenv("TALIX_API_URL", "http://localhost:8000")
API_KEY = os.getenv("TALIX_CONNECTION_KEY", "")
...
SYSTEM_PROMPT = (
    ...
    "All messages are screened by Project 73 Security guardrails for safety."
)
...
def scan_input(text: str) -> dict:
    """Run the user message through Project 73 Security input guardrails."""
    try:
        resp = requests.post(
...
def scan_output(text: str, prompt: str) -> dict:
    """Run the AI response through Project 73 Security output guardrails."""
    try:
        resp = requests.post(
...
def chat():
    ...
    if not input_scan.get("is_valid", True):
        ...
        return jsonify({
            ...
            "message": "Your message was blocked by Project 73 Security guardrails.",
        })
    ...
    if not output_scan.get("is_valid", True):
        ...
        return jsonify({
            ...
            "message": "The AI response was blocked by Project 73 Security guardrails.",
        })
...
if __name__ == "__main__":
    ...
    logger.info("Project 73 chatbot running on http://localhost:%d", port)
