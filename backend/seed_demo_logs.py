...
async def seed():
    ...
    async with async_session_maker() as session:
        ...
        for e in DEMO_LOGS:
            ...
            outcome = (
                "reask"    if not e["is_valid"] and any(v == "reask" for v in actions.values()) else
                "blocked"  if not e["is_valid"] else
                "fixed"    if e["fix_applied"] else
                "monitored" if any(v == "monitored" for v in actions.values()) else
                "pass"
            )
