...
async def _sync_guardrail_defaults():
    ...
    async with async_session_maker() as session:
        ...
        marker = (await session.execute(
            _select(PlatformSetting).where(PlatformSetting.key == "guardrails_defaults_v10")
        )).scalar_one_or_none()
        if marker:
            return
        ...
        for g in all_gs:
            ...
            if entry:
                # v10: sync params with LangSmith CTF-derived attack patterns — preserve user's is_active toggles
        ...
        session.add(PlatformSetting(key="guardrails_defaults_v10", value="applied"))
        await session.commit()
        invalidate_cache()
        if updated:
            logger.info("Synced %d guardrail configs to v10 defaults (LangSmith CTF attack patterns).", updated)
