...
async def _sync_guardrail_defaults():
    ...
    async with async_session_maker() as session:
        ...
        marker = (await session.execute(
            _select(PlatformSetting).where(PlatformSetting.key == "guardrails_defaults_v7")
        )).scalar_one_or_none()
        if marker:
            return
        ...
        session.add(PlatformSetting(key="guardrails_defaults_v7", value="applied"))
        await session.commit()
        invalidate_cache()
        if updated:
            logger.info("Synced %d guardrail param sets to v7 defaults (Promptfoo patterns added).", updated)
