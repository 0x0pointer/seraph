...
async def get_billing_stats(
    ...
    """Revenue overview: plan distribution, MRR estimate, invoice totals."""
    ...
    invoice_stats = {
        row[0]: {"count": row[1], "total": float(row[2] or 0)} for row in inv_rows
    }

    total_invoiced = sum(v["total"] for v in invoice_stats.values())
    total_paid = invoice_stats.get("paid", {}).get("total", 0.0)
    total_open = invoice_stats.get("open", {}).get("total", 0.0)
...
async def list_all_invoices(
    ...
    """List all invoices across all users, with user info."""
    from app.models.billing import Invoice
