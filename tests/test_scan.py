...
class TestScanPrompt:
    def test_clean_input_returns_valid(self, client):
        with patch(
    ...
    def test_blocked_input_returns_invalid(self, client):
        with patch(
            "app.services.scanner_engine.run_input_scan",
            new_callable=AsyncMock,
    ...
    def test_monitored_input_still_valid(self, client):
        with patch(
            "app.services.scanner_engine.run_input_scan",
            new_callable=AsyncMock,
    ...
    def test_fix_applied_flag(self, client):
        with patch(
            "app.services.scanner_engine.run_input_scan",
            new_callable=AsyncMock,
    ...
    def test_missing_text_returns_422(self, client):
        resp = client.post("/api/scan/prompt", json={})
        assert resp.status_code == 422


class TestScanOutput:
    def test_clean_output_returns_valid(self, client):
        with patch(
    ...
    def test_blocked_output_returns_invalid(self, client):
        with patch(
            "app.services.scanner_engine.run_output_scan",
            new_callable=AsyncMock,
    ...
    def test_output_without_prompt(self, client):
        with patch(
            "app.services.scanner_engine.run_output_scan",
            new_callable=AsyncMock,
...
class TestScanGuard:
    def test_clean_messages_not_flagged(self, client):
        with patch(
    ...
    def test_malicious_messages_flagged(self, client):
        with patch(
            "app.services.scanner_engine.run_guard_scan",
            new_callable=AsyncMock,
    ...
    def test_guard_with_breakdown_flag(self, client):
        with patch(
            "app.services.scanner_engine.run_guard_scan",
            new_callable=AsyncMock,
            return_value=_guard_flagged_result(),
    ...
    def test_guard_without_breakdown_returns_null(self, client):
        with patch(
            "app.services.scanner_engine.run_guard_scan",
            new_callable=AsyncMock,
            return_value=_guard_clean_result(),
